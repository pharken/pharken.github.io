...
const tableColumns = [ { title: 'Site' },{ title: 'Site ID' },{ title: 'Engagement' },{ title: 'Sections' },{ title: 'Desc' },{ title: 'Launch' } ];

const projectDataset = [
    ["sandbox", "49985427", "---", [ "routing", "parkinglot" ], "not sure, various playground demos" ],
...
let projectMain = function (){
    ...
    let projectTable = new DataTable('#projectTable', {
        ...
        columnDefs: [
            {
                data: null,
                defaultContent: '<button class="btn btn-secondary" type = "button">go</button>',
                targets: -1
            }
        ]
    });
...
*/
    ...
    projectTable.on('click', 'button', function (e) {
        ...
        let href = `${hrefDomain}/project/${data[0]}.html?${hrefParams}`;
